...
final class Router<EndPoint: EndPointType, T: Codable>: NSObject, NetworkRouter, URLSessionDelegate {

    // MARK: - Properties

    private var task: URLSessionTask?
    public typealias NetworkRouterCompletion = ((Result<T?>) -> Void)
    ...
    func checkNetwork(completion: @escaping (NetworkState) -> Void) {
...
extension URLRequest {
    ...
    Configures the URL request for `multipart/form-data`.
     ...
     - Note: The default `httpMethod` is `GET`,
        and `GET` requests do not typically have a response body.
     ...
     */
    mutating func setMultipartBodyImage(_ image: UIImage) throws {
        ...
        // End the raw http request data, note that there is 2 extra dash ("-") at the end,
        // this is to indicate the end of the data
